...
def point_hausdroff_distance(set_a,set_b):
       
    #Calculate min from a -> b
    point_hausdroff_distance_a = []
    for a in set_a:
        min_d = 9999
        for b in set_b:
            distance = dist(a,b)
            if(distance<min_d):
                min_d=distance
        point_hausdroff_distance_a.append(min_d)

    #Calculate min from b -> a
    point_hausdroff_distance_b = []
    for b in set_b:
        min_d = 9999
        for a in set_a:
            distance = dist(b,a) 
            if(min_d > distance):
                min_d=distance
        point_hausdroff_distance_b.append(min_d)

    return  max(max(point_hausdroff_distance_a),max(point_hausdroff_distance_b))
